...
/* useEffect Section Start *//* Descriptions before each one include the dependencies */
	...
	useEffect(()=> {
		...
		const gameContainerElement: HTMLElement | null = document.querySelector('#game');
		if (gameContainerElement) {
			dispatch({
				...
				payload: {
					...
					cHeight:	(gameContainerElement.offsetWidth/8)*1.4,
					cMidHeight:	((gameContainerElement.offsetWidth/8)*1.4)/2,
					cWidth: 	gameContainerElement.offsetWidth/8,
					cMidWidth:	(gameContainerElement.offsetWidth/8)/2
				},
			});
		}
